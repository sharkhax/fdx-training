...
import com.drobot.beam.schema.AvroSchemaUtil;
...
public class RecordsEnrichTransform extends PTransform<PCollection<GenericRecord>, PCollection<GenericRecord>> {
    ...
    private static final Schema RECORD_SCHEMA = RecordSchema.getAvroRecordSchema();
    ...
    public PCollection<GenericRecord> expand(PCollection<GenericRecord> input) {
        ...
        PCollection<KV<Long, GenericRecord>> kvUsersData = prepareUsersData(input.getPipeline());
        ...
        return grouped
                ...
                .setCoder(AvroCoder.of(RECORD_SCHEMA));
    }
    ...
    private PCollection<KV<Long, GenericRecord>> prepareUsersData(Pipeline pipeline) {
        JdbcIO.RowMapper<GenericRecord> rowMapper = getRowMapper();
        Coder<GenericRecord> coder = AvroCoder.of(RECORD_SCHEMA);
        return pipeline.begin()
                .apply("Read users data", new DatabaseReader<>(SQL_STATEMENT, rowMapper, coder))
    ...
    private JdbcIO.RowMapper<GenericRecord> getRowMapper() {
        return rs -> {
            GenericRecordBuilder builder = AvroSchemaUtil.createRecordBuilderWithDefaultValues(RECORD_SCHEMA);
            return builder
